...
- 2016.07至今	QQ空间前端开发
    ...
    - 空间服务端截图服务的负责人。
        ...
        - 字符串`hash`模块改造为`c++`实现，开启`cmem`缓存下测试发现，在字符串很大的情况下，用`js`实现`hash`效率低，字符串长度对响应的时间有直接影响，`hash`计算属于`cpu`密集型，适合放到`c++`计算，目前真在实现。(*更正：实现了`c++`扩展版本，对比`js`和`c++`扩展，`js`效率只是略低于`c++`，排除了`js`的`hash`计算效率低的可能性，需要继续排查其他原因*)
